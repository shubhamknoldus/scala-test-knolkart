...
class AddUserServiceTest extends FunSuite {
  ...
  val wrongUserName = "pawan@gmail.com"
  val correctUserName = "shyam@gmail.com"
  val wrongPassword = "pawan@123"
  val correctPassword = "Shyam@123"
  ...
  test("it should return false in following cases 1") {
    assert(!addUserServiceObj.authenticateUser(wrongUserName, wrongPassword))
  }
  test("it should return false in following cases 2") {
    assert(!addUserServiceObj.authenticateUser(correctUserName, wrongPassword))
  }
  test("it should return false in following cases 3") {
    assert(!addUserServiceObj.authenticateUser(wrongUserName, correctPassword))
  }
  test("it should return false in following cases 4") {
    assert(addUserServiceObj.authenticateUser(correctUserName, correctPassword))
  }
}
